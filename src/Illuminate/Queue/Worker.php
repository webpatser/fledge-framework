    ...
    protected function listenForSignals($connectionName = null, $queue = null)
    {
        if ($this->supportsRevoltSignals()) {
            foreach ([SIGQUIT, SIGTERM, SIGINT] as $signal) {
                $this->signalWatchers[] = EventLoop::onSignal($signal, function () use ($signal, $connectionName, $queue) {
            ...
            $this->signalWatchers[] = EventLoop::onSignal(SIGUSR2, function () use ($connectionName, $queue) {
                $this->paused = true;

                $this->events->dispatch(new WorkerPausing($connectionName, $queue));
            });
            $this->signalWatchers[] = EventLoop::onSignal(SIGCONT, function () use ($connectionName, $queue) {
                $this->paused = false;
            ...
            pcntl_signal(SIGUSR2, function () use ($connectionName, $queue) {
                $this->paused = true;
